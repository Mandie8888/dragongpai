...
import { Sparkles, Play, Info, Target, Crown } from "lucide-react";
...
const labels = {
  ...
  description: { en: "About", tc: "關於", sc: "关于" },
  disclaimer: {
  ...
  selectConfig: { en: "Select Configuration", tc: "選擇配置", sc: "选择配置" },
  // Elon
  ...
  autoDesc: { en: "No configuration needed — runs 500,000 virtual draws automatically.", tc: "無需配置 — 自動進行五十萬次虛擬抽獎。", sc: "无需配置 — 自动进行五十万次虚拟抽奖。" },
...
const CharacterProfileModal = ({ character, open, onClose, onSelect }: Props) => {
  ...
  // If no character or modal not open, return null
  ...
  const charDescription = character.description?.[lang] || character.description?.en || '';

  // Build config & check validity
  const buildConfig = (): CharacterConfig | null => {
    // Determine config type based on character ID
    ...
    if (id === "elon") {
      return bankerNumbers.length >= 1 ? { type: "banker", bankerNumbers } : null;
    }
    if (id === "gambling") {
      return pattern ? { type: "pattern", pattern } : null;
    }
    if (id === "achelois") {
      return distMode && distRatio ? { type: "distribution", mode: distMode, ratio: distRatio } : null;
    }
    if (id === "aladdin") {
      return color && colorRatio ? { type: "color", color, colorRatio } : null;
    }
    // For characters with no config (dragon-master, phoenix-trend, tiger-volatility)
    return { type: "none" };
  };
  ...
  // Determine if character needs config
  const needsConfig = ["elon", "gambling", "achelois", "aladdin"].includes(character.id);
        ...
        <div className="p-6 space-y-4">
          <DialogHeader className="items-center text-center">
            <div className="w-20 h-20 rounded-full overflow-hidden border-2 border-white/20 mx-auto mb-2 shadow-lg">
              <img 
                src={avatarMap[character.avatar]} 
                alt={charName} 
                className="w-full h-full object-cover" 
              />
            </div>
            <DialogTitle className="text-2xl font-extrabold text-white">{charName}</DialogTitle>
          ...
          {charDescription && (
            <div className="rounded-xl border border-white/10 bg-white/[0.03] p-3 space-y-1">
              <h4 className="text-[10px] font-bold text-[#d4af37] uppercase tracking-wider flex items-center gap-1">
                <Info size={12} /> {l("description", lang)}
              </h4>
              <p className="text-xs text-white/80 leading-relaxed italic">{charDescription}</p>
            </div>
          )}
          ...
          <div className="rounded-xl border border-white/10 bg-white/[0.03] p-3 space-y-1">
            <h4 className="text-[10px] font-bold text-[#d4af37] uppercase tracking-wider flex items-center gap-1">
              <Target size={12} /> {l("method", lang)}
            </h4>
            <p className="text-sm font-semibold text-white">{charMethod}</p>
          </div>
          ...
          {needsConfig && (
            <div className="space-y-3">
              <h4 className="text-xs font-bold text-[#d4af37] flex items-center gap-1">
                <Crown size={14} /> {l("selectConfig", lang)}
              </h4>
              ...
              {character.id === "gambling" && (
                <div className="rounded-xl border border-red-400/30 bg-red-950/20 p-3 space-y-2">
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    >
                      <div className="text-sm font-bold text-white">{l("hotPatterns", lang)}</div>
                    ...
                    >
                      <div className="text-sm font-bold text-white">{l("coldPatterns", lang)}</div>
              ...
              {/* Achelois - Distribution */}
              {character.id === "achelois" && (
              ...
              {character.id === "aladdin" && (
                <div className="rounded-xl border border-emerald-400/30 bg-emerald-950/20 p-3 space-y-3">
                  ...
                  {color && (
                    <>
                      <p className="text-[10px] text-white/70 mt-2">{l("colorRatioDesc", lang)}</p>
                      <div className="flex flex-wrap gap-2 justify-center">
          ...
          {/* Select button */}
          // In CharacterProfileModal.tsx, find the button section and update:

{/* Select button */}
<Button
  onClick={handleSelect}
  disabled={!isReady && needsConfig}
...
>
  {character.id === "lucky-star" ? (
    <><Sparkles size={16} className="mr-2" />{lang === "en" ? "Use this Simulation Methodology" : lang === "tc" ? "使用此模擬方法" : "使用此模拟方法"}</>
  ) : (
